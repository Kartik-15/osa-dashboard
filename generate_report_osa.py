...
<title id="page-title">Shelf Availability Report</title>
<meta name="description" content="Shelf availability KPI dashboard — before vs after comparison"/>
<style>
*,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
:root{
  --bg:#0D1117;--surf:#161B27;--surf2:#1C2235;--bdr:#252D3D;--bdr2:#1E2536;
  --amber:#F5A623;--teal:#2DD4A7;--red:#E05252;--blue:#5B8DEF;--purple:#9B7FE8;
  --txt:#E8EBF4;--txt2:#8892A8;--txt3:#464F65;--r:10px;
}
html,body{min-height:100%;background:var(--bg);color:var(--txt);font-family:system-ui,-apple-system,sans-serif;font-size:14px;line-height:1.5}

/* ── layout ── */
#app{display:grid;grid-template-columns:234px 1fr;grid-template-rows:52px 1fr;min-height:100vh}
#topbar{grid-column:1/-1;display:flex;align-items:center;gap:8px;padding:0 14px;border-bottom:1px solid var(--bdr);background:var(--surf);position:sticky;top:0;z-index:50;flex-wrap:nowrap}
#sidebar{border-right:1px solid var(--bdr);background:var(--surf);padding:16px 12px;overflow-y:auto}
#main{overflow-y:auto;padding:18px;display:flex;flex-direction:column;gap:16px}

/* ── topbar ── */
.logo-dot{width:26px;height:26px;background:var(--amber);border-radius:6px;display:flex;align-items:center;justify-content:center;font-weight:800;font-size:11px;color:#0D1117;flex-shrink:0}
.logo-lbl{font-weight:700;font-size:13px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;max-width:200px}
.logo-sub{font-size:10px;color:var(--txt2);letter-spacing:.3px;text-transform:uppercase}
.mode-badge{padding:2px 8px;border-radius:20px;font-size:10px;font-weight:700;letter-spacing:.3px;text-transform:uppercase;white-space:nowrap;flex-shrink:0}
.mode-before{background:#0A1F0A;color:var(--teal);border:1px solid #1A4A1A}
.mode-full{background:#201500;color:var(--amber);border:1px solid #4A2A00}
.tb-sep{width:1px;height:18px;background:var(--bdr);flex-shrink:0}
.tb-btn{padding:4px 10px;border-radius:6px;border:1px solid var(--bdr);background:transparent;color:var(--txt2);font-size:11px;cursor:pointer;white-space:nowrap;flex-shrink:0}
.tb-btn:hover{background:var(--bdr2);color:var(--txt)}
.tb-btn.primary{color:var(--amber);border-color:rgba(245,166,35,.35);font-weight:600}
.tb-btn.primary:hover{background:rgba(245,166,35,.1)}
.tb-btn.blue{color:var(--blue);border-color:rgba(91,141,239,.3)}
.tb-btn.purple{color:var(--purple);border-color:rgba(155,127,232,.3)}
.tb-btn.teal{color:var(--teal);border-color:rgba(45,212,167,.3)}
.tb-btn.teal:hover{background:rgba(45,212,167,.1)}
.tb-right{margin-left:auto;display:flex;align-items:center;gap:8px}

/* ── sidebar ── */
.sb-label{font-size:10px;font-weight:700;letter-spacing:1px;text-transform:uppercase;color:var(--txt3);margin-bottom:7px}
.sb-sec{margin-bottom:18px}
.ctrl{width:100%;background:var(--bg);border:1px solid var(--bdr);border-radius:6px;padding:6px 9px;color:var(--txt);font-size:12px;outline:none}
.ctrl:focus{border-color:var(--amber)}
select.ctrl{appearance:none;cursor:pointer}
.chip{display:inline-flex;align-items:center;gap:4px;background:#201500;border:1px solid #3A2000;border-radius:20px;padding:2px 8px;font-size:10px;color:var(--amber);margin:2px}
.chip-x{cursor:pointer;opacity:.45}.chip-x:hover{opacity:1}
.reset-btn{width:100%;padding:5px;border-radius:6px;border:1px solid var(--bdr);background:transparent;color:var(--txt2);font-size:11px;cursor:pointer;margin-top:6px}
.reset-btn:hover{background:var(--bdr2);color:var(--txt)}
.qc-prog-bar{height:4px;border-radius:3px;background:var(--bdr);margin-top:8px;overflow:hidden}
.qc-prog-fill{height:100%;border-radius:3px;background:linear-gradient(90deg,var(--amber),var(--teal));transition:width .3s}
.qc-summary{font-size:10px;color:var(--txt2);margin-top:4px;display:flex;justify-content:space-between}

/* ── KPI cards ── */
.kpi-row{display:grid;grid-template-columns:repeat(auto-fit,minmax(130px,1fr));gap:11px}
.kpi-card{background:var(--surf);border:1px solid var(--bdr);border-radius:var(--r);padding:14px 16px}
.kpi-card.accent{border-color:rgba(245,166,35,.2);background:linear-gradient(135deg,#141000 0%,#161B27 100%)}
.kpi-lbl{font-size:10px;font-weight:700;letter-spacing:.8px;text-transform:uppercase;color:var(--txt2);margin-bottom:7px}
.kpi-val{font-size:28px;font-weight:800;letter-spacing:-1px;font-variant-numeric:tabular-nums;line-height:1}
.kpi-sub{font-size:11px;color:var(--txt2);margin-top:4px}
.kpi-filter{cursor:pointer;transition:filter .15s,box-shadow .15s}
.kpi-filter:hover{filter:brightness(1.12)}
.kpi-filter.kf-active{box-shadow:0 0 0 2px currentColor}
.kpi-filter-hint{font-size:9px;font-weight:400;letter-spacing:0;text-transform:none;opacity:.4;margin-left:3px}

/* ── charts ── */
.charts-grid{display:grid;grid-template-columns:1fr 1fr;gap:11px}
.charts-row{display:grid;grid-template-columns:1fr 1fr;gap:11px}
.card{background:var(--surf);border:1px solid var(--bdr);border-radius:var(--r);padding:16px}
.card-title{font-size:13px;font-weight:700;margin-bottom:2px}
.card-sub{font-size:11px;color:var(--txt2);margin-bottom:12px}
.hist-wrap{display:flex;align-items:flex-end;gap:3px;height:150px;padding-bottom:24px;position:relative}
.hbar-col{display:flex;flex-direction:column;align-items:center;justify-content:flex-end;flex:1;height:100%;cursor:pointer}
.hbar{width:100%;border-radius:3px 3px 0 0;min-height:2px;transition:opacity .15s,filter .15s}
.hbar-col:hover .hbar{filter:brightness(1.25)}
.hbar-col.dimmed .hbar{opacity:.18}
.hbar-col.selected .hbar{filter:brightness(1.4);outline:2px solid var(--amber)}
.hbar-lbl{font-size:9px;color:var(--txt2);white-space:nowrap;position:absolute;bottom:2px}
.hbar-cnt{font-size:9px;color:var(--txt2);margin-bottom:2px}
.trend-svg-wrap{height:150px}
.trend-svg-wrap svg{width:100%;height:100%;display:block}
.period-tag{display:inline-block;padding:1px 7px;border-radius:10px;font-size:10px;font-weight:700;letter-spacing:.3px;margin-left:7px;vertical-align:middle}
.period-tag.before{background:rgba(45,212,167,.12);color:var(--teal)}
.period-tag.after{background:rgba(245,166,35,.12);color:var(--amber)}

/* ── section ── */
.sec-hdr{display:flex;align-items:center;gap:8px;margin-bottom:12px;flex-wrap:wrap}
.sec-title{font-size:14px;font-weight:700}
.sec-count{font-size:11px;color:var(--txt2);margin-left:auto}
.tab-row{display:flex;gap:3px}
.tab{padding:3px 10px;border-radius:20px;border:1px solid var(--bdr);background:transparent;color:var(--txt2);font-size:11px;cursor:pointer}
.tab.active{background:var(--amber);color:#0D1117;border-color:var(--amber);font-weight:600}

/* ── tables ── */
.tbl-wrap{overflow-x:auto;border-radius:var(--r);border:1px solid var(--bdr)}
table{width:100%;border-collapse:collapse;font-size:12px}
thead th{background:#0F1520;padding:8px 11px;text-align:left;font-size:10px;font-weight:700;letter-spacing:.5px;text-transform:uppercase;color:var(--txt2);border-bottom:1px solid var(--bdr);white-space:nowrap;cursor:pointer;user-select:none}
thead th:hover{color:var(--txt)}
.sort-ic{opacity:.28;margin-left:3px}
th.sorted .sort-ic{opacity:1;color:var(--amber)}
tbody tr{border-bottom:1px solid var(--bdr2);cursor:pointer;transition:background .1s}
tbody tr:last-child{border-bottom:none}
tbody tr:hover{background:rgba(255,255,255,.022)}
tbody td{padding:8px 11px;vertical-align:middle;font-variant-numeric:tabular-nums}
.td-name{font-weight:500;max-width:220px}
.td-name small{display:block;font-size:10px;color:var(--txt2);font-weight:400}
.badge{display:inline-block;padding:2px 7px;border-radius:11px;font-size:10px;font-weight:600}
.bg{background:rgba(45,212,167,.12);color:var(--teal)}
.ba{background:rgba(245,166,35,.12);color:var(--amber)}
.br{background:rgba(224,82,82,.12);color:var(--red)}
.bgy{background:rgba(136,146,168,.1);color:var(--txt2)}
.rbar-wrap{display:flex;align-items:center;gap:5px}
.rbar{height:4px;border-radius:3px;background:var(--bdr);width:55px;flex-shrink:0;overflow:hidden}
.rbar-fill{height:100%;border-radius:3px;background:var(--amber)}
.rval{font-size:11px;color:var(--txt2);width:32px;text-align:right;font-variant-numeric:tabular-nums}
.pager{display:flex;align-items:center;gap:7px;padding:10px 11px;border-top:1px solid var(--bdr);font-size:11px;color:var(--txt2)}
.pg-btn{padding:3px 8px;border-radius:5px;border:1px solid var(--bdr);background:transparent;color:var(--txt2);cursor:pointer;font-size:11px}
.pg-btn:hover{background:var(--bdr2);color:var(--txt)}
.pg-btn:disabled{opacity:.28;cursor:default}
.pg-info{margin:0 auto}

/* ── QC indicators ── */
.qc-cell{display:flex;align-items:center;gap:5px}
.qc-dot{width:7px;height:7px;border-radius:50%;flex-shrink:0}
.qc-unchecked{background:var(--txt3)}
.qc-opened{background:var(--amber);box-shadow:0 0 5px rgba(245,166,35,.4)}
.qc-done{background:var(--teal);box-shadow:0 0 5px rgba(45,212,167,.3)}
.qc-lbl{font-size:10px}
tr.row-opened{border-left:2px solid rgba(245,166,35,.45)}
tr.row-done{border-left:2px solid rgba(45,212,167,.4)}
tr.row-done td{color:var(--txt2)}
tr.row-done .td-name{color:var(--txt)}
tr.row-flagged{border-left:2px solid rgba(224,82,82,.55)!important}
tr.sku-row-active{background:rgba(245,166,35,.07)!important;border-left:2px solid var(--amber)}

/* ── pending ── */
.pending{background:var(--surf);border:1px solid var(--bdr);border-radius:var(--r);padding:34px;text-align:center}
.pending-icon{font-size:26px;margin-bottom:9px}
.pending-title{font-size:14px;font-weight:700;margin-bottom:5px}
.pending-desc{font-size:12px;color:var(--txt2);max-width:370px;margin:0 auto}
.pending-path{font-family:ui-monospace,monospace;background:var(--bg);border:1px solid var(--bdr);border-radius:5px;padding:5px 11px;font-size:11px;color:var(--amber);display:inline-block;margin-top:9px}

/* ── drill panel ── */
#drill{position:fixed;right:0;top:52px;bottom:0;width:360px;background:var(--surf2);border-left:1px solid var(--bdr);padding:16px;overflow-y:auto;transform:translateX(100%);transition:transform .22s ease;z-index:200}
#drill.open{transform:translateX(0)}
.drill-x{position:absolute;top:12px;right:12px;cursor:pointer;color:var(--txt2);font-size:17px;width:24px;height:24px;display:flex;align-items:center;justify-content:center;border-radius:4px}
.drill-x:hover{background:var(--bdr);color:var(--txt)}
.drill-name{font-size:14px;font-weight:700;padding-right:28px;margin-bottom:2px}
.drill-meta{font-size:11px;color:var(--txt2);margin-bottom:14px}
.ds-title{font-size:10px;font-weight:700;letter-spacing:.8px;text-transform:uppercase;color:var(--txt3);margin-bottom:7px}
.ds{margin-bottom:16px}
.pill{display:inline-block;background:rgba(245,166,35,.1);border:1px solid rgba(245,166,35,.18);border-radius:4px;padding:2px 7px;font-size:11px;color:var(--amber);margin:2px}
.pill.c{background:rgba(91,141,239,.08);border-color:rgba(91,141,239,.18);color:var(--blue)}
.ilink{display:block;color:var(--blue);text-decoration:none;font-size:12px;margin:3px 0;padding:4px 8px;border-radius:5px;transition:background .1s}
.ilink:hover{background:rgba(91,141,239,.1)}
.ilink.visited{color:var(--teal)}.ilink.visited::before{content:"✓ "}
.qc-status-bar{display:flex;align-items:center;gap:8px;padding:8px 10px;border-radius:7px;font-size:12px;font-weight:600;margin-bottom:12px}
.qc-status-bar.unchecked{background:rgba(70,79,101,.15);color:var(--txt2)}
.qc-status-bar.opened{background:rgba(245,166,35,.1);color:var(--amber);border:1px solid rgba(245,166,35,.2)}
.qc-status-bar.done{background:rgba(45,212,167,.1);color:var(--teal);border:1px solid rgba(45,212,167,.2)}
.action-row{display:flex;gap:6px;margin-bottom:10px}
.act-btn{flex:1;padding:7px 5px;border-radius:7px;border:1px solid var(--bdr);font-size:12px;font-weight:600;cursor:pointer;transition:opacity .15s;text-align:center}
.act-btn:hover{opacity:.82}
.act-done{background:var(--teal);color:#0D1117;border-color:var(--teal)}
.act-undo{background:transparent;color:var(--txt2);font-size:11px}
.act-flag{background:rgba(224,82,82,.12);color:var(--red);border-color:rgba(224,82,82,.25)}
.act-unflag{background:transparent;color:var(--txt3);font-size:11px}
#flag-form{background:rgba(224,82,82,.07);border:1px solid rgba(224,82,82,.2);border-radius:7px;padding:10px;margin-bottom:10px}
#flag-form textarea{width:100%;background:var(--bg);border:1px solid var(--bdr);border-radius:5px;padding:6px 8px;color:var(--txt);font-size:12px;font-family:inherit;resize:vertical;min-height:68px;outline:none;margin-bottom:8px}
#flag-form textarea:focus{border-color:var(--red)}
.flag-actions{display:flex;gap:6px}
.flag-save{flex:1;padding:6px;border-radius:5px;border:none;background:var(--red);color:#fff;font-size:12px;font-weight:600;cursor:pointer}
.flag-cancel{padding:6px 10px;border-radius:5px;border:1px solid var(--bdr);background:transparent;color:var(--txt2);font-size:11px;cursor:pointer}
.flag-display{background:rgba(224,82,82,.07);border:1px solid rgba(224,82,82,.2);border-radius:7px;padding:9px 10px;margin-bottom:10px}
.flag-display-title{font-size:10px;font-weight:700;letter-spacing:.6px;text-transform:uppercase;color:var(--red);margin-bottom:5px}
.flag-note-text{font-size:12px;color:var(--txt);line-height:1.45}

/* ── UPLOAD MODAL ── */
#upload-overlay{position:fixed;inset:0;background:rgba(0,0,0,.7);backdrop-filter:blur(4px);z-index:500;display:flex;align-items:center;justify-content:center;padding:20px}
#upload-overlay.hidden{display:none}
#upload-box{background:var(--surf);border:1px solid var(--bdr);border-radius:14px;width:100%;max-width:680px;max-height:90vh;overflow-y:auto;padding:28px}
.upload-hdr{display:flex;align-items:flex-start;justify-content:space-between;margin-bottom:20px}
.upload-title{font-size:18px;font-weight:800;letter-spacing:-.4px}
.upload-sub{font-size:12px;color:var(--txt2);margin-top:3px}
.upload-close{cursor:pointer;color:var(--txt2);font-size:20px;line-height:1;padding:2px 6px;border-radius:4px}
.upload-close:hover{background:var(--bdr);color:var(--txt)}
.upload-step{margin-bottom:20px}
.upload-step-title{font-size:11px;font-weight:700;letter-spacing:.8px;text-transform:uppercase;color:var(--txt3);margin-bottom:10px}
.file-zones{display:grid;grid-template-columns:1fr 1fr;gap:12px}
.file-zone{border:2px dashed var(--bdr);border-radius:10px;padding:20px 14px;text-align:center;cursor:pointer;transition:border-color .15s,background .15s;position:relative}
.file-zone:hover,.file-zone.drag-over{border-color:var(--amber);background:rgba(245,166,35,.05)}
.file-zone.has-file{border-color:var(--teal);border-style:solid;background:rgba(45,212,167,.05)}
.fz-icon{font-size:22px;margin-bottom:6px}
.fz-label{font-size:12px;font-weight:600;margin-bottom:3px}
.fz-sub{font-size:11px;color:var(--txt2)}
.fz-name{font-size:11px;color:var(--teal);font-weight:600;margin-top:6px;word-break:break-all}
.fz-input{position:absolute;inset:0;opacity:0;cursor:pointer}
.col-grid{display:grid;grid-template-columns:1fr 1fr;gap:10px}
.col-field{display:flex;flex-direction:column;gap:4px}
.col-field label{font-size:11px;color:var(--txt2);font-weight:600}
.col-field select,.col-field input{background:var(--bg);border:1px solid var(--bdr);border-radius:6px;padding:6px 8px;color:var(--txt);font-size:12px;outline:none;width:100%}
.col-field select:focus,.col-field input:focus{border-color:var(--amber)}
.self-val-row{display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-top:8px}
.upload-actions{display:flex;gap:10px;margin-top:20px}
.up-btn{flex:1;padding:10px;border-radius:8px;border:none;font-size:13px;font-weight:700;cursor:pointer;transition:opacity .15s}
.up-btn:hover{opacity:.85}
.up-btn.go{background:var(--amber);color:#0D1117}
.up-btn.cancel{background:transparent;border:1px solid var(--bdr);color:var(--txt2);font-size:12px}
#col-mapper{display:none}
.progress-note{font-size:11px;color:var(--txt2);margin-top:8px;text-align:center}
.up-prog-wrap{margin-top:14px;display:none;flex-direction:column;gap:10px}
.up-prog-row{display:flex;flex-direction:column;gap:4px}
.up-prog-label{font-size:11px;color:var(--txt2);text-align:left}
.up-prog-bar{height:6px;border-radius:3px;background:var(--bdr);overflow:hidden}
.up-prog-fill{height:100%;border-radius:3px;background:linear-gradient(90deg,var(--amber),var(--teal));transition:width .25s;width:0%}
.up-prog-fill.indeterminate{width:35%!important;animation:up-prog-indeterminate 1.1s ease-in-out infinite}
@keyframes up-prog-indeterminate{0%{margin-left:-35%}100%{margin-left:100%}}

/* ── view toggle ── */
.view-toggle-row{display:none;align-items:center;gap:10px}
.view-toggle{display:flex;align-items:center;border:1px solid var(--bdr);border-radius:8px;overflow:hidden;background:var(--bg);flex-shrink:0}
.vt-btn{padding:5px 20px;border:none;background:transparent;color:var(--txt2);font-size:12px;font-weight:600;cursor:pointer;transition:background .15s,color .15s;letter-spacing:.2px}
.vt-btn:hover{color:var(--txt);background:rgba(255,255,255,.04)}
.vt-btn.vt-active{background:var(--amber);color:#0D1117}
.vt-sep{width:1px;height:14px;background:var(--bdr)}
.vt-context{font-size:11px;color:var(--txt2)}
.vt-context strong{color:var(--txt);font-variant-numeric:tabular-nums}

/* ── both-mode comparison extra columns ── */
.col-after{display:none}
#app.view-both .col-after{display:table-cell}
.delta-up{background:rgba(45,212,167,.13);color:var(--teal)}
.delta-down{background:rgba(224,82,82,.13);color:var(--red)}
.delta-same{background:rgba(136,146,168,.08);color:var(--txt2)}
.aft-val{color:var(--txt2);font-size:12px}
.aft-val.aft-zero{color:var(--red)}
.aft-val.aft-pos{color:var(--teal)}

/* ── toast ── */
.toast{position:fixed;bottom:18px;left:50%;transform:translateX(-50%);background:#1C2235;border:1px solid var(--bdr);border-radius:8px;padding:8px 16px;font-size:13px;color:var(--txt);z-index:600;opacity:0;transition:opacity .2s;pointer-events:none;white-space:nowrap}
.toast.show{opacity:1}

@media print{
  #app{display:block}
  #sidebar,#drill,#upload-overlay,.tb-btn,.pager{display:none!important}
  #main{padding:0}
  body{background:#fff;color:#000}
  .kpi-card,.card,.tbl-wrap{border:1px solid #ddd}
}
@media(max-width:860px){
  #app{grid-template-columns:1fr}
  #sidebar{display:none}
  .kpi-row,.charts-row{grid-template-columns:1fr 1fr}
  .file-zones,.col-grid,.self-val-row{grid-template-columns:1fr}
}
</style>
</head>
<body>
...
<header id="topbar">
  ...
  <div>
    <div class="logo-lbl" id="top-project">Shelf Availability Report</div>
    <div class="logo-sub" id="top-period">—</div>
  </div>
  <span id="mode-badge" class="mode-badge mode-before">Loading…</span>
  <div class="tb-sep"></div>
  <button class="tb-btn primary" onclick="openUpload()">⬆ Load Files</button>
  <button class="tb-btn teal" onclick="exportExcel()">⬇ Export Excel</button>
  <button class="tb-btn blue" onclick="exportQC()">⬇ Export QC</button>
  <label class="tb-btn purple" style="cursor:pointer">⬆ Import QC<input type="file" accept=".json" style="display:none" onchange="importQC(this)"/></label>
  <div class="tb-sep"></div>
...
<aside id="sidebar">
  <div class="sb-sec">
    <div class="sb-label">Search Store</div>
    <input id="s-search" class="ctrl" type="text" placeholder="Store name…"/>
  </div>
  <div class="sb-sec">
    <div class="sb-label">Filter by SKU</div>
    <select id="s-sku" class="ctrl"><option value="">All own-brand SKUs</option></select>
  </div>
  <div class="sb-sec">
    <div class="sb-label">SKU Count</div>
    <select id="s-range" class="ctrl">
      <option value="">Any</option>
      <option value="0">0 — gap stores</option>
      <option value="1-3">1–3 SKUs</option>
      <option value="4-6">4–6 SKUs</option>
      <option value="7-10">7–10 SKUs</option>
      <option value="11+">11+ SKUs</option>
    </select>
  </div>
  <div class="sb-sec">
    <div class="sb-label">QC Status</div>
    <select id="s-qc" class="ctrl">
    ...
    <div class="qc-prog-bar"><div class="qc-prog-fill" id="qc-fill" style="width:0%"></div></div>
    <div class="qc-summary"><span id="qc-done-lbl">0 done</span><span id="qc-pct">0%</span></div>
  </div>
  <div class="sb-sec">
...
<main id="main">
  <div class="kpi-row" id="kpi-row">
    <div class="kpi-card accent">
      <div class="kpi-lbl">Own-Brand Availability</div>
      <div class="kpi-val" id="k-avail" style="color:var(--amber)">—</div>
      <div class="kpi-sub" id="k-avail-s"></div>
    </div>
    <div class="kpi-card">
      <div class="kpi-lbl">Stores Visited</div>
      <div class="kpi-val" id="k-stores">—</div>
      <div class="kpi-sub" id="k-stores-s"></div>
    </div>
    <div class="kpi-card">
      <div class="kpi-lbl">Stores — No Own Brand</div>
      <div class="kpi-val" style="color:var(--red)" id="k-none">—</div>
      <div class="kpi-sub" id="k-none-s"></div>
    </div>
    <div class="kpi-card">
      <div class="kpi-lbl">Own-Brand SKUs</div>
      <div class="kpi-val" id="k-skus">—</div>
      <div class="kpi-sub" id="k-skus-s"></div>
    </div>
    <div class="kpi-card kpi-filter" id="kf-done" onclick="toggleKpiFilter('done')"
         style="border-color:rgba(45,212,167,.2);background:linear-gradient(135deg,#001410 0%,#161B27 100%)">
      <div class="kpi-lbl" style="color:var(--teal)">QC Complete <span class="kpi-filter-hint">↗ filter</span></div>
      <div class="kpi-val" id="k-qc-done" style="color:var(--teal)">0</div>
      <div class="kpi-sub" id="k-qc-done-s"></div>
    </div>
    <div class="kpi-card kpi-filter" id="kf-flagged" onclick="toggleKpiFilter('flagged')"
         style="border-color:rgba(224,82,82,.2);background:linear-gradient(135deg,#140500 0%,#161B27 100%)">
      <div class="kpi-lbl" style="color:var(--red)">Flags Raised <span class="kpi-filter-hint">↗ filter</span></div>
      <div class="kpi-val" id="k-flagged" style="color:var(--red)">0</div>
      <div class="kpi-sub" id="k-flagged-s">no issues raised</div>
    </div>
  </div>

  <div class="view-toggle-row" id="view-toggle-row">
    <div class="view-toggle">
      <button class="vt-btn vt-active" id="vtb-both"   onclick="switchView('both')">Both</button>
      <div class="vt-sep"></div>
      <button class="vt-btn" id="vtb-before" onclick="switchView('before')">Before</button>
      <div class="vt-sep"></div>
      <button class="vt-btn" id="vtb-after"  onclick="switchView('after')">After</button>
    </div>
    <div class="vt-context" id="vt-context"></div>
  </div>

  <div class="charts-grid" id="charts-grid">
    <div class="card" id="ch-bh">
      <div class="card-title">SKU Count Distribution <span class="period-tag before">Before</span></div>
      <div class="card-sub" id="hist-before-sub"></div>
      <div class="hist-wrap" id="hist-before-wrap"></div>
    </div>
    <div class="card" id="ch-ah">
      <div class="card-title">SKU Count Distribution <span class="period-tag after">After</span></div>
      <div class="card-sub" id="hist-after-sub"></div>
      <div class="hist-wrap" id="hist-after-wrap"></div>
    </div>
    <div class="card" id="ch-bt">
      <div class="card-title">Availability Trend <span class="period-tag before">Before</span></div>
      <div class="card-sub" id="trend-before-sub"></div>
      <div class="trend-svg-wrap" id="trend-before-wrap"></div>
    </div>
    <div class="card" id="ch-at">
      <div class="card-title">Availability Trend <span class="period-tag after">After</span></div>
      <div class="card-sub" id="trend-after-sub"></div>
      <div class="trend-svg-wrap" id="trend-after-wrap"></div>
    </div>
  </div>

  <div class="card">
    <div class="sec-hdr">
      <div class="sec-title">Store-Level View</div>
      <div class="sec-count" id="st-count"></div>
    </div>
    <div class="tbl-wrap">
      <table>
        <thead><tr>
          <th onclick="sortSt('last_date')" data-col="last_date">Date <span class="sort-ic">↕</span></th>
          <th onclick="sortSt('name')" data-col="name">Store <span class="sort-ic">↕</span></th>
          <th onclick="sortSt('n_self_skus')" data-col="n_self_skus" class="sorted"><span id="th-own-skus-lbl">Own SKUs</span> <span class="sort-ic">↓</span></th>
          <th class="col-after">After SKUs</th>
          <th class="col-after">Δ SKUs</th>
          <th onclick="sortSt('n_comp_skus')" data-col="n_comp_skus">Comp. SKUs <span class="sort-ic">↕</span></th>
          <th onclick="sortSt('n_visits')" data-col="n_visits">Visits <span class="sort-ic">↕</span></th>
          <th onclick="sortSt('n_images')" data-col="n_images">Images <span class="sort-ic">↕</span></th>
          <th>Status</th>
          <th>QC</th>
        </tr></thead>
      ...
      <div class="pager">
        <button class="pg-btn" id="pg-prev" onclick="chPage(-1)">‹ Prev</button>
        <span class="pg-info" id="pg-info"></span>
        <button class="pg-btn" id="pg-next" onclick="chPage(1)">Next ›</button>
      </div>
    </div>
  </div>
  ...
  <div class="card">
    <div class="sec-hdr">
      <div class="sec-title">SKU Availability</div>
      <div class="tab-row">
        <button class="tab active" onclick="skuTab('self',this)">Own-Brand SKUs</button>
        <button class="tab" onclick="skuTab('comp',this)">Competitor SKUs</button>
      </div>
      <div class="sec-count" id="sku-count"></div>
    </div>
    <div class="tbl-wrap">
      <table>
        <thead><tr>
          <th onclick="sortSku('sku')" data-col="sku">SKU <span class="sort-ic">↕</span></th>
          <th onclick="sortSku('stores')" data-col="stores" class="sorted"><span id="th-sku-stores-lbl">Stores</span> <span class="sort-ic">↓</span></th>
          <th onclick="sortSku('rate')" data-col="rate"><span id="th-sku-rate-lbl">Availability</span> <span class="sort-ic">↕</span></th>
          <th class="col-after">After stores</th>
          <th class="col-after">After avail.</th>
          <th class="col-after">Δ pp</th>
        </tr></thead>
  ...
  <div id="after-sec"></div>
</main>
</div>

<!-- drill panel -->
<div id="drill">
  <span class="drill-x" onclick="closeDrill()">✕</span>
...
<div id="upload-overlay" class="hidden">
  <div id="upload-box">
    <div class="upload-hdr">
      <div>
        <div class="upload-title">Load Data Files</div>
        <div class="upload-sub">OSA files are required; Gallery dumps add shelf images. Before + After enables gap analysis.</div>
      </div>
      <span class="upload-close" onclick="closeUpload()">✕</span>
    </div>

    <div class="upload-step">
      <div class="upload-step-title">1 — Select files</div>
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px">
        <div style="font-size:10px;font-weight:700;letter-spacing:.8px;text-transform:uppercase;color:var(--teal);padding:2px 0">Before</div>
        <div style="font-size:10px;font-weight:700;letter-spacing:.8px;text-transform:uppercase;color:var(--amber);padding:2px 0">After</div>
        <div class="file-zone" id="fz-before" ondragover="dzOver(event,'fz-before')" ondragleave="dzLeave('fz-before')" ondrop="dzDrop(event,'before')">
          <input class="fz-input" type="file" accept=".xlsx,.xls,.csv" onchange="fileChosen(this,'before')"/>
          <div class="fz-icon">📊</div>
          <div class="fz-label">OSA Data <span style="color:var(--red)">*</span></div>
          <div class="fz-sub">On-shelf availability file</div>
          <div class="fz-name" id="fn-before"></div>
        </div>
        <div class="file-zone" id="fz-after" ondragover="dzOver(event,'fz-after')" ondragleave="dzLeave('fz-after')" ondrop="dzDrop(event,'after')">
          <input class="fz-input" type="file" accept=".xlsx,.xls,.csv" onchange="fileChosen(this,'after')"/>
          <div class="fz-icon">📊</div>
          <div class="fz-label">OSA Data</div>
          <div class="fz-sub">Optional — for gap analysis</div>
          <div class="fz-name" id="fn-after"></div>
        </div>
        <div class="file-zone" id="fz-before_gal" ondragover="dzOver(event,'fz-before_gal')" ondragleave="dzLeave('fz-before_gal')" ondrop="dzDrop(event,'before_gal')">
          <input class="fz-input" type="file" accept=".xlsx,.xls,.csv" onchange="fileChosen(this,'before_gal')"/>
          <div class="fz-icon">🖼</div>
          <div class="fz-label">Gallery Dump</div>
          <div class="fz-sub">Optional — adds shelf images</div>
          <div class="fz-name" id="fn-before_gal"></div>
        </div>
        <div class="file-zone" id="fz-after_gal" ondragover="dzOver(event,'fz-after_gal')" ondragleave="dzLeave('fz-after_gal')" ondrop="dzDrop(event,'after_gal')">
          <input class="fz-input" type="file" accept=".xlsx,.xls,.csv" onchange="fileChosen(this,'after_gal')"/>
          <div class="fz-icon">🖼</div>
          <div class="fz-label">Gallery Dump</div>
          <div class="fz-sub">Optional — adds shelf images</div>
          <div class="fz-name" id="fn-after_gal"></div>
        </div>
      </div>
    </div>

    <div id="col-mapper" class="upload-step">
      <div class="upload-step-title">2 — Map columns <span style="font-weight:400;color:var(--txt2)">(auto-detected from Before file — same mapping applied to After)</span></div>
      <div class="col-grid">
        <div class="col-field">
          <label>Store name column *</label>
          <select id="cm-store"></select>
        </div>
        <div class="col-field">
          <label>SKU / Product name column *</label>
          <select id="cm-sku"></select>
        </div>
        <div class="col-field">
          <label>Brand type column *</label>
          <select id="cm-brand"></select>
        </div>
        <div class="col-field">
          <label>Date column *</label>
          <select id="cm-date"></select>
        </div>
        <div class="col-field">
          <label>On Shelf Availability column * (0/1)</label>
          <select id="cm-osa"></select>
        </div>
        <div class="col-field">
          <label>Image URL column (optional)</label>
          <select id="cm-link"></select>
        </div>
        <div class="col-field">
          <label>Store ID column (optional)</label>
          <select id="cm-id"></select>
        </div>
      </div>
      <div class="self-val-row">
        <div class="col-field">
          <label>Value meaning "own brand" in brand column *</label>
          <input id="cm-self-val" type="text" placeholder='e.g. "self"'/>
        </div>
        <div class="col-field">
          <label>Value meaning "competitor"</label>
          <input id="cm-comp-val" type="text" placeholder='e.g. "competitor"'/>
        </div>
      </div>
    </div>

    <div class="upload-actions">
      <button class="up-btn cancel" onclick="closeUpload()">Cancel</button>
      <button class="up-btn go" id="up-go-btn" onclick="processUpload()" disabled>Process →</button>
    </div>
    <div class="up-prog-wrap" id="up-prog-wrap">
      <div class="up-prog-row" id="up-prog-before">
        <div class="up-prog-label" id="up-prog-label-before">Before</div>
        <div class="up-prog-bar"><div class="up-prog-fill" id="up-prog-fill-before"></div></div>
      </div>
      <div class="up-prog-row" id="up-prog-after" style="display:none">
        <div class="up-prog-label" id="up-prog-label-after">After</div>
        <div class="up-prog-bar"><div class="up-prog-fill" id="up-prog-fill-after"></div></div>
      </div>
    </div>
    <div class="progress-note" id="up-note"></div>
  </div>
</div>
...
// ── background parsing/compute worker source (built at generation time — see
// WORKER_LOGIC_JS in generate_report_osa.py). Blob-ed into a real Worker so large
// uploads parse off the UI thread instead of freezing the tab.
const WORKER_SRC = __WORKER_SRC_JSON__;

// ── active data globals (replaced when user uploads) ─────────────────────────
let B = null, A = null, D = null, HAS_AFTER = false;
let viewMode = "both";
function activeData(){ return viewMode==="after" && A ? A : B; }

// Cross-reference maps for "both" mode comparison columns
let afterStoreMap = {};
let afterSkuMap   = {};

// ── QC localStorage ───────────────────────────────────────────────────────────
const QC_KEY = "shelf_qc_v1";
function loadQC(){ try{ return JSON.parse(localStorage.getItem(QC_KEY)||"{}"); }catch{ return {}; } }
function saveQC(s){ try{ localStorage.setItem(QC_KEY,JSON.stringify(s)); }catch{} }
function qcGet(n){ return loadQC()[n]||{status:"unchecked",visited_links:[],flagged:false,flag_note:""}; }
function qcMutate(n,fn){ const qc=loadQC(),r=qc[n]||{status:"unchecked",visited_links:[],flagged:false,flag_note:""}; fn(r); qc[n]=r; saveQC(qc); }
function qcOpen(n){  qcMutate(n,r=>{ if(r.status==="unchecked"){ r.status="opened"; r.opened_at=new Date().toISOString(); }}); }
function qcDone(n){  qcMutate(n,r=>{ r.status="done"; r.done_at=new Date().toISOString(); }); }
function qcUndo(n){  qcMutate(n,r=>{ r.status="opened"; delete r.done_at; }); }
function qcFlag(n,note){ qcMutate(n,r=>{ r.flagged=true; r.flag_note=note||""; r.flagged_at=new Date().toISOString(); if(r.status==="unchecked"){ r.status="opened"; r.opened_at=new Date().toISOString(); }}); }
function qcUnflag(n){ qcMutate(n,r=>{ r.flagged=false; r.flag_note=""; delete r.flagged_at; }); }
function qcVisitLink(n,url){ qcMutate(n,r=>{ if(!r.visited_links) r.visited_links=[]; if(!r.visited_links.includes(url)) r.visited_links.push(url); if(r.status==="unchecked"){ r.status="opened"; r.opened_at=new Date().toISOString(); }}); }

// Unique key per (store, audit_date) visit — used for per-visit QC tracking
function visitKey(s){ return s.name+"||"+(s.date||""); }

function qcStats(){
  const qc=loadQC(),total=B?(B.total_visits||B.total_stores):0; let done=0,opened=0,flagged=0;
  Object.values(qc).forEach(r=>{ if(r.status==="done")done++; else if(r.status==="opened")opened++; if(r.flagged)flagged++; });
  return {total,done,opened,flagged};
}
function updateProgress(){
  if(!B) return;
  const {total,done,flagged}=qcStats();
  const pct=total?Math.round(100*done/total):0;
  document.getElementById("qc-fill").style.width=pct+"%";
  document.getElementById("qc-done-lbl").textContent=done+" done"+(flagged?", "+flagged+" flagged":"");
  document.getElementById("qc-pct").textContent=pct+"%";
  document.getElementById("k-qc-done").textContent=done;
  document.getElementById("k-qc-done-s").textContent="of "+total+" audits ("+pct+"%)";
  document.getElementById("k-flagged").textContent=flagged;
  document.getElementById("k-flagged-s").textContent=flagged===0?"no issues raised":flagged===1?"1 audit flagged":flagged+" audits flagged";
}

// ── export filtered data to Excel (1 row per image) ────────────────────────────
function exportExcel(){
  if(!B){ showToast("No data loaded"); return; }
  if(typeof XLSX==="undefined"){ showToast("Excel export unavailable in this build"); return; }
  const qc=loadQC();
  const rows=filtered();
  if(!rows.length){ showToast("No rows match the current filters"); return; }
  const showAfterCols=HAS_AFTER && viewMode==="both";
  const out=[];
  rows.forEach(s=>{
    const rec=qc[visitKey(s)]||{}, status=rec.status||"unchecked", isFlagged=!!rec.flagged;
    const aftSt=showAfterCols?afterStoreMap[s.name]:null;
    const base={
      "Store Name":            s.name,
      "Client ID":             s.client_id||"",
      "Shop ID":               s.shop_id||"",
      "Audit Date":            s.date||"",
      "Total Visits":          s.n_visits,
      "Self SKUs Count":       s.n_self_skus,
      "Self SKUs":             s.self_skus.join(", "),
      "Competitor SKUs Count": s.n_comp_skus,
      "Competitor SKUs (top 10)": s.comp_skus.join(", "),
      "Total Images":          s.n_images,
    };
    if(showAfterCols){
      base["After Self SKUs Count"]=aftSt?aftSt.n_self_skus:"";
      base["Delta (After − Before)"]=aftSt?(aftSt.n_self_skus-s.n_self_skus):"";
    }
    base["QC Status"]=QC_LBL[status]||status;
    base["Flagged"]=isFlagged?"Yes":"No";
    base["Flag Note"]=rec.flag_note||"";

    const links=(s.all_links&&s.all_links.length)?s.all_links:[""];
    links.forEach((link,i)=>{
      out.push({...base, "Image #":link?i+1:"", "Image URL":link});
    });
  });
  const ws=XLSX.utils.json_to_sheet(out);
  const wb=XLSX.utils.book_new();
  XLSX.utils.book_append_sheet(wb,ws,"Filtered Stores");
  const stamp=new Date().toISOString().slice(0,10);
  XLSX.writeFile(wb, `osa_export_${viewMode}_${stamp}.xlsx`);
  showToast(`Exported ${out.length.toLocaleString()} image rows from ${rows.length.toLocaleString()} visits`);
}

// ── export / import QC ────────────────────────────────────────────────────────
function exportQC(){
  const blob=new Blob([JSON.stringify({_v:"shelf_qc_v1",_exported:new Date().toISOString(),state:loadQC()},null,2)],{type:"application/json"});
  const url=URL.createObjectURL(blob);
  const a=document.createElement("a"); a.href=url; a.download="qc_state.json";
  document.body.appendChild(a); a.click(); document.body.removeChild(a);
  setTimeout(()=>URL.revokeObjectURL(url),100);
  showToast("QC state exported");
}
function importQC(input){
  const file=input.files[0]; if(!file) return;
  const r=new FileReader(); r.onload=e=>{
    let incoming;
    try{ const p=JSON.parse(e.target.result); incoming=p.state||p; }
    catch{ showToast("Error: invalid file"); return; }
    saveQC({...loadQC(),...incoming});
    renderStoreTable(); updateProgress(); if(_drillStore) renderDrill(_drillStore);
    showToast("Imported — "+Object.keys(incoming).length+" stores merged");
  }; r.readAsText(file); input.value="";
}

// ── ui state ──────────────────────────────────────────────────────────────────
let q="",skuF="",rangeF="",qcF="",histBucket=null,histBucketView="before";
let skuTabCur="self",stSort={col:"n_self_skus",dir:-1},skuSort={col:"stores",dir:-1};
let page=1; const PER=50;

function _syncViewClass(){
  const app=document.getElementById("app");
  app.classList.toggle("view-both", viewMode==="both");
  const lbl=document.getElementById("th-own-skus-lbl");
  if(lbl) lbl.textContent=(viewMode==="both"&&HAS_AFTER)?"Before SKUs":"Own SKUs";
  const sl=document.getElementById("th-sku-stores-lbl");
  if(sl) sl.textContent=(viewMode==="both"&&HAS_AFTER)?"Before stores":"Stores";
  const rl=document.getElementById("th-sku-rate-lbl");
  if(rl) rl.textContent=(viewMode==="both"&&HAS_AFTER)?"Before avail.":"Availability";
}

// ── init ───────────────────────────────────────────────────────────────────────
window.addEventListener("DOMContentLoaded",()=>{
  // Always start fresh and require a file upload — this is a generic portal for
  // any project, not just the dataset it happened to be generated with.
  openUpload();
  document.getElementById("s-search").addEventListener("input",e=>{q=e.target.value.toLowerCase();page=1;go();});
  document.getElementById("s-sku").addEventListener("change",e=>{skuF=e.target.value;page=1;go();});
  document.getElementById("s-range").addEventListener("change",e=>{rangeF=e.target.value;histBucket=null;page=1;go();refreshHist();});
  document.getElementById("s-qc").addEventListener("change",e=>{qcF=e.target.value;page=1;go();syncKpiRings();});
});

// ── load (or reload) a full dataset ──────────────────────────────────────────
function loadDataset(before, after, delta, hasAfter){
  const sn=before.sku_names, cn=before.comp_names;
  before.stores.forEach(s=>{ s.self_skus=(s.si||[]).map(i=>sn[i]); s.comp_skus=(s.ci||[]).map(i=>cn[i]); s.last_date=s.date||""; });
  if(after && after.stores){
    const asn=after.sku_names, acn=after.comp_names;
    after.stores.forEach(s=>{ s.self_skus=(s.si||[]).map(i=>asn[i]); s.comp_skus=(s.ci||[]).map(i=>acn[i]); s.last_date=s.date||""; });
  }

  B=before; A=after; D=delta; HAS_AFTER=hasAfter;
  viewMode = HAS_AFTER ? "both" : "before";

  afterStoreMap = {};
  afterSkuMap   = {};
  if(A){
    // Keep the most-recent After visit per store for comparison columns
    A.stores.forEach(s=>{ const p=afterStoreMap[s.name]; if(!p||(s.date||"")>(p.date||"")) afterStoreMap[s.name]=s; });
    (A.self_skus||[]).forEach(s=>{ afterSkuMap[s.sku]=s; });
  }

  document.getElementById("top-project").textContent = B.project_name||"Availability Report";
  document.title = (B.project_name||"Availability Report")+" — Report";
  document.getElementById("mode-badge").textContent   = HAS_AFTER?"Before + After":"Before Only";
  document.getElementById("mode-badge").className     = "mode-badge "+(HAS_AFTER?"mode-full":"mode-before");

  const toggleRow = document.getElementById("view-toggle-row");
  toggleRow.style.display = HAS_AFTER ? "flex" : "none";
  ["both","before","after"].forEach(m=>{
    const el=document.getElementById("vtb-"+m); if(el) el.classList.toggle("vt-active",m===viewMode);
  });
  _applyChartVisibility();
  _syncViewClass();
  if(HAS_AFTER) _updateVtContext();

  q="";skuF="";rangeF="";qcF="";histBucket=null; page=1;
  ["s-search","s-sku","s-range","s-qc"].forEach(id=>{ const el=document.getElementById(id); if(el) el.value=""; });

  refreshKpiCards(B);
  populateSkuDropdown(B);
  buildHist(); buildTrend();
  renderSkuTable(); renderStoreTable(); renderAfter(); updateProgress();
}

function refreshKpiCards(data){
  document.getElementById("top-period").textContent   = data.date_range||"";
  document.getElementById("k-avail").textContent   = data.overall_avail_rate+"%";
  document.getElementById("k-avail-s").textContent = data.stores_with_brunch+" of "+data.total_stores+" stores stocking own brand";
  document.getElementById("k-stores").textContent  = data.total_stores.toLocaleString();
  document.getElementById("k-stores-s").textContent= "across "+data.date_trend.length+" visit dates";
  document.getElementById("k-none").textContent    = data.stores_no_brunch.toLocaleString();
  document.getElementById("k-none-s").textContent  = "zero own-brand SKUs detected";
  document.getElementById("k-skus").textContent    = data.total_self_skus;
  document.getElementById("k-skus-s").textContent  = data.total_comp_skus+" competitor SKUs tracked";
}

function populateSkuDropdown(data){
  const sel=document.getElementById("s-sku");
  sel.innerHTML='<option value="">All own-brand SKUs</option>';
  data.self_skus.forEach(s=>{ const o=document.createElement("option"); o.value=s.sku; o.textContent=s.sku+" ("+s.rate+"%)"; sel.appendChild(o); });
}

function _updateVtContext(){
  if(!HAS_AFTER||!D) return;
  const sign=D.avail_delta>=0?"+":"";
  const col=D.avail_delta>=0?"var(--teal)":"var(--red)";
  document.getElementById("vt-context").innerHTML=
    `Availability: Before <strong>${D.avail_before}%</strong> → After <strong>${D.avail_after}%</strong>&ensp;<strong style="color:${col}">${sign}${D.avail_delta}pp</strong>`;
}

function _applyChartVisibility(){
  const showBefore = viewMode==="both"||viewMode==="before";
  const showAfter  = (viewMode==="both"||viewMode==="after") && HAS_AFTER;
  document.getElementById("ch-bh").style.display = showBefore ? "" : "none";
  document.getElementById("ch-bt").style.display = showBefore ? "" : "none";
  document.getElementById("ch-ah").style.display = showAfter  ? "" : "none";
  document.getElementById("ch-at").style.display = showAfter  ? "" : "none";
}

function switchView(mode){
  if(mode===viewMode||!B) return;
  if(mode==="after"&&!A) return;
  viewMode=mode;
  ["both","before","after"].forEach(m=>{
    const el=document.getElementById("vtb-"+m); if(el) el.classList.toggle("vt-active",m===mode);
  });
  histBucket=null; histBucketView="before"; skuF=""; rangeF="";
  ...
  refreshKpiCards(data);
  populateSkuDropdown(data);
  page=1; renderSkuTable(); renderStoreTable(); renderChips();
}

// ── histogram ─────────────────────────────────────────────────────────────────
function colFor(b){ return b===0?"var(--red)":b<=3?"#D4850A":b<=7?"var(--amber)":"var(--teal)"; }

function _buildHistFor(data, wrapId, subId, isBothAfter){
  if(!data) return;
  document.getElementById(subId).textContent=`${data.date_range} — click a bar to filter`;
  const wrap=document.getElementById(wrapId); wrap.innerHTML="";
  const H=data.histogram, maxC=Math.max(...H.map(h=>h.count));
  H.forEach(h=>{
    const ph=Math.max(4,Math.round((h.count/maxC)*124));
    const col=document.createElement("div"); col.className="hbar-col"; col.dataset.bucket=h.bucket;
    col.innerHTML=`<div class="hbar-cnt">${h.count}</div><div class="hbar" style="height:${ph}px;background:${colFor(h.bucket)}"></div>`;
    const lbl=document.createElement("div"); lbl.className="hbar-lbl"; lbl.textContent=h.bucket; col.appendChild(lbl);
    col.addEventListener("click",()=>{
      const thisView = isBothAfter ? "after" : "before";
      const newBucket = (histBucket===h.bucket && histBucketView===thisView) ? null : h.bucket;
      histBucket=newBucket; histBucketView=thisView;
      rangeF=""; document.getElementById("s-range").value="";
      page=1; go(); refreshHist();
    });
...
function buildHist(){
  if(!B) return;
  _buildHistFor(B, "hist-before-wrap", "hist-before-sub", false);
  if(A) _buildHistFor(A, "hist-after-wrap", "hist-after-sub", viewMode==="both");
}
function refreshHist(){
  const beforeActive = histBucket!==null && histBucketView==="before";
  const afterActive  = histBucket!==null && histBucketView==="after";
  document.querySelectorAll("#hist-before-wrap .hbar-col").forEach(c=>{
    const b=parseInt(c.dataset.bucket); c.classList.remove("dimmed","selected");
    if(beforeActive) c.classList.add(b===histBucket?"selected":"dimmed");
  });
  document.querySelectorAll("#hist-after-wrap .hbar-col").forEach(c=>{
    const b=parseInt(c.dataset.bucket); c.classList.remove("dimmed","selected");
    if(afterActive) c.classList.add(b===histBucket?"selected":"dimmed");
  });
}

// ── trend SVG ─────────────────────────────────────────────────────────────────
function _buildTrendFor(data, wrapId, subId, color){
  if(!data) return;
  document.getElementById(subId).textContent=`% stores with ≥1 own-brand SKU present, by visit date (${data.date_range})`;
  const wrap=document.getElementById(wrapId);
  const W=wrap.offsetWidth||400, H=145, trend=data.date_trend, n=trend.length;
  if(n<2){ wrap.innerHTML='<div style="color:var(--txt3);font-size:11px;padding:20px;text-align:center">Not enough dates</div>'; return; }
  const pL=32,pR=8,pT=8,pB=26,cW=W-pL-pR,cH=H-pT-pB;
  const px=i=>pL+(i/(n-1))*cW, py=v=>pT+cH-(v/100)*cH;
  let grid="";
  [0,25,50,75,100].forEach(v=>{ const y=py(v);
    grid+=`<line x1="${pL}" y1="${y}" x2="${W-pR}" y2="${y}" stroke="rgba(255,255,255,0.055)" stroke-width="1"/>`;
    grid+=`<text x="${pL-4}" y="${y+3}" text-anchor="end" font-size="9" fill="#8892A8">${v}</text>`;
  });
  const step=Math.max(1,Math.ceil(n/7));
  let xlbls="";
  for(let i=0;i<n;i+=step){ xlbls+=`<text x="${px(i).toFixed(1)}" y="${H-3}" text-anchor="middle" font-size="9" fill="#8892A8">${trend[i].date}</text>`; }
  const pts=trend.map((t,i)=>`${px(i).toFixed(1)},${py(t.rate).toFixed(1)}`).join(" ");
  const area=`M${px(0).toFixed(1)},${py(trend[0].rate).toFixed(1)} `+trend.map((t,i)=>`L${px(i).toFixed(1)},${py(t.rate).toFixed(1)}`).join(" ")+` L${px(n-1).toFixed(1)},${py(0).toFixed(1)} L${px(0).toFixed(1)},${py(0).toFixed(1)}Z`;
  const gid="tg"+wrapId;
  const dots=trend.map((t,i)=>`<circle cx="${px(i).toFixed(1)}" cy="${py(t.rate).toFixed(1)}" r="2.5" fill="${color}"><title>${t.date}: ${t.rate}% (${t.stores_with_brunch}/${t.stores_visited})</title></circle>`).join("");
  wrap.innerHTML=`<svg viewBox="0 0 ${W} ${H}" xmlns="http://www.w3.org/2000/svg">
    ...
    <polyline points="${pts}" fill="none" stroke="${color}" stroke-width="2" stroke-linejoin="round"/>${dots}${xlbls}</svg>`;
}
function buildTrend(){
  if(!B) return;
  _buildTrendFor(B, "trend-before-wrap", "trend-before-sub", "#2DD4A7");
  if(A) _buildTrendFor(A, "trend-after-wrap", "trend-after-sub", "#F5A623");
}

// ── SKU table ─────────────────────────────────────────────────────────────────
function skuTab(tab,btn){ skuTabCur=tab; document.querySelectorAll(".tab").forEach(b=>b.classList.remove("active")); btn.classList.add("active"); skuSort={col:"stores",dir:-1}; renderSkuTable(); }
function sortSku(col){ skuSort.dir=skuSort.col===col?-skuSort.dir:-1; skuSort.col=col; renderSkuTable(); }
function renderSkuTable(){
  if(!B) return;
  const data=activeData();
  const skus=[...(skuTabCur==="self"?data.self_skus:data.comp_skus)];
  const {col,dir}=skuSort;
  skus.sort((a,b)=>typeof a[col]==="string"?dir*a[col].localeCompare(b[col]):dir*(a[col]-b[col]));
  ...
  skus.forEach(s=>{
    const cls=s.rate>=30?"bg":s.rate>=10?"ba":"br", lbl=s.rate>=30?"Good":s.rate>=10?"Low":"Rare";
    const tr=document.createElement("tr");

    let aftCells = '<td class="col-after">—</td><td class="col-after">—</td><td class="col-after">—</td>';
    if(skuTabCur==="self" && afterSkuMap[s.sku]){
      const as = afterSkuMap[s.sku];
      const dp = Math.round(10*(as.rate - s.rate))/10;
      const dCls = dp>0?"delta-up":dp<0?"delta-down":"delta-same";
      const dTxt = dp>0?"+"+dp:String(dp);
      aftCells=`<td class="col-after">${as.stores}</td><td class="col-after">${as.rate}%</td><td class="col-after"><span class="badge ${dCls}">${dTxt}pp</span></td>`;
    } else if(skuTabCur==="self"){
      aftCells=`<td class="col-after">0</td><td class="col-after">0%</td><td class="col-after"><span class="badge delta-down">-${s.rate}pp</span></td>`;
    }

    tr.innerHTML=`<td class="td-name">${esc(s.sku)}</td><td>${s.stores}</td>
      <td><div class="rbar-wrap"><div class="rbar"><div class="rbar-fill" style="width:${Math.min(s.rate,100)}%"></div></div><span class="rval">${s.rate}%</span><span class="badge ${cls}">${lbl}</span></div></td>${aftCells}`;
    if(skuTabCur==="self"){
      tr.style.cursor="pointer";
      tr.title="Click to filter stores stocking this SKU";
      tr.addEventListener("click",()=>{
        const selecting = skuF!==s.sku;
        skuF = selecting ? s.sku : "";
        document.getElementById("s-sku").value=skuF;
        page=1; go();
        tbody.querySelectorAll("tr").forEach(r=>r.classList.remove("sku-row-active"));
        if(selecting){
          tr.classList.add("sku-row-active");
          const stCard=document.getElementById("st-body")?.closest(".card");
          if(stCard) stCard.scrollIntoView({behavior:"smooth",block:"start"});
          showToast("Filtering stores for: "+s.sku);
        }
      });
    }
...
// ── store table ───────────────────────────────────────────────────────────────
const QC_COL={unchecked:"var(--txt3)",opened:"var(--amber)",done:"var(--teal)"};
const QC_LBL={unchecked:"Unchecked",opened:"Opened",done:"Done"};
function sortSt(col){ stSort.dir=stSort.col===col?-stSort.dir:-1; stSort.col=col; page=1; renderStoreTable(); }

function filtered(){
  if(!B) return [];
  const qc=loadQC();
  let s=[...activeData().stores];
  if(q)    s=s.filter(x=>x.name.toLowerCase().includes(q));
  if(skuF) s=s.filter(x=>x.self_skus.includes(skuF));
  if(histBucket!==null){
    if(histBucketView==="after" && viewMode==="both"){
      s=s.filter(x=>{ const a=afterStoreMap[x.name]; return a&&a.n_self_skus===histBucket; });
    } else {
      s=s.filter(x=>x.n_self_skus===histBucket);
    }
  }
  else if(rangeF){
    if(rangeF==="0")    s=s.filter(x=>x.n_self_skus===0);
    else if(rangeF==="1-3")  s=s.filter(x=>x.n_self_skus>=1&&x.n_self_skus<=3);
    else if(rangeF==="4-6")  s=s.filter(x=>x.n_self_skus>=4&&x.n_self_skus<=6);
    else if(rangeF==="7-10") s=s.filter(x=>x.n_self_skus>=7&&x.n_self_skus<=10);
    else if(rangeF==="11+")  s=s.filter(x=>x.n_self_skus>=11);
  }
  if(qcF){
    s=s.filter(x=>{ const r=qc[visitKey(x)]||{}; const st=r.status||"unchecked";
      if(qcF==="flagged") return !!r.flagged;
      if(qcF==="not-done") return st!=="done";
      return st===qcF; });
  }
  const {col,dir}=stSort;
  s.sort((a,b)=>typeof a[col]==="string"?dir*a[col].localeCompare(b[col]):dir*(a[col]-b[col]));
  return s;
}

function renderStoreTable(){
  if(!B) return;
  const qc=loadQC(), rows=filtered(), total=rows.length;
  const pages=Math.max(1,Math.ceil(total/PER));
  page=Math.min(page,pages);
  const slice=rows.slice((page-1)*PER,page*PER);
  document.getElementById("st-count").textContent=total.toLocaleString()+" audit visits";
  document.getElementById("pg-info").textContent=`Page ${page} of ${pages} (${total})`;
  document.getElementById("pg-prev").disabled=page<=1;
  document.getElementById("pg-next").disabled=page>=pages;
  const tbody=document.getElementById("st-body"); tbody.innerHTML="";
  slice.forEach(s=>{
    const rec=qc[visitKey(s)]||{}, st=rec.status||"unchecked", fl=!!rec.flagged;
    const acls=s.n_self_skus===0?"br":s.n_self_skus<=3?"ba":"bg";
    const albl=s.n_self_skus===0?"Gap":s.n_self_skus<=3?"Thin":"Present";
    const tr=document.createElement("tr");
    tr.className=["row-"+st, fl?"row-flagged":""].filter(Boolean).join(" ");

    const aftSt  = afterStoreMap[s.name];
    const aftN   = aftSt ? aftSt.n_self_skus : null;
    const delta  = aftN !== null ? aftN - s.n_self_skus : null;
    const dCls   = delta===null?"":delta>0?"delta-up":delta<0?"delta-down":"delta-same";
    const dTxt   = delta===null?"—":delta>0?"+"+delta:String(delta);
    const aftCls = aftN===null?"aft-val":aftN===0?"aft-val aft-zero":"aft-val aft-pos";

    tr.innerHTML=`<td style="white-space:nowrap;font-size:11px;color:var(--txt2);font-variant-numeric:tabular-nums">${esc(s.last_date||'—')}</td>
      <td class="td-name">${esc(s.name)}<small>${esc(s.client_id||"")}</small></td>
      <td><strong>${s.n_self_skus}</strong></td>
      <td class="col-after"><span class="${aftCls}">${aftN??'—'}</span></td>
      <td class="col-after"><span class="badge ${dCls}">${dTxt}</span></td>
      <td>${s.n_comp_skus}</td>
      <td>${s.n_visits}</td><td>${s.n_images}</td>
      <td><span class="badge ${acls}">${albl}</span></td>
      <td><div class="qc-cell"><div class="qc-dot qc-${st}"></div><span class="qc-lbl" style="color:${QC_COL[st]}">${QC_LBL[st]}</span>${fl?'<span title="'+esc(rec.flag_note||"Flagged")+'" style="font-size:11px;margin-left:2px">🚩</span>':''}</div></td>`;
    tr.addEventListener("click",()=>openDrill(s));
    tbody.appendChild(tr);
  });
}
function chPage(d){ page+=d; renderStoreTable(); }
function go(){ page=1; renderStoreTable(); renderChips(); }

// ── KPI filter buttons ────────────────────────────────────────────────────────
function toggleKpiFilter(val){
  qcF=qcF===val?"":val;
  document.getElementById("s-qc").value=qcF;
  page=1; go(); syncKpiRings();
}
function syncKpiRings(){
  document.getElementById("kf-done").classList.toggle("kf-active",qcF==="done");
  document.getElementById("kf-flagged").classList.toggle("kf-active",qcF==="flagged");
}

// ── drill panel ───────────────────────────────────────────────────────────────
let _drillStore=null;
function openDrill(s){ _drillStore=s; qcOpen(visitKey(s)); renderDrill(s); document.getElementById("drill").classList.add("open"); renderStoreTable(); updateProgress(); }
function renderDrill(s){
  const rec=qcGet(visitKey(s)), st=rec.status, fl=!!rec.flagged, visited=rec.visited_links||[];
  const stBar=`<div class="qc-status-bar ${st}">${{unchecked:"○ Unchecked",opened:"◐ In Review",done:"✓ QC Complete"}[st]}${fl?'<span style="margin-left:auto">🚩</span>':""}</div>`;
  const doneBtn=st==="done"?`<button class="act-btn act-undo" onclick="doDrillUndo()">↩ Not done</button>`:`<button class="act-btn act-done" onclick="doDrillDone()">✓ Mark Complete</button>`;
  const flagBtn=fl?`<button class="act-btn act-unflag" onclick="doDrillUnflag()">Remove flag</button>`:`<button class="act-btn act-flag" onclick="showFlagForm()">🚩 Flag Issue</button>`;
  const flagDisplay=fl?`<div class="flag-display"><div class="flag-display-title">🚩 Issue${rec.flagged_at?" · "+new Date(rec.flagged_at).toLocaleDateString():""}</div><div class="flag-note-text">${esc(rec.flag_note||"No note")}</div></div>`:"";
  const skuPills=s.self_skus.length?s.self_skus.map(sk=>`<span class="pill">${esc(sk)}</span>`).join(""):`<span style="color:var(--txt3);font-size:11px">None detected</span>`;
  const compPills=s.comp_skus.length?s.comp_skus.map(sk=>`<span class="pill c">${esc(sk)}</span>`).join("")+(s.n_comp_skus>s.comp_skus.length?`<span style="color:var(--txt3);font-size:11px"> +${s.n_comp_skus-s.comp_skus.length} more</span>`:""):`<span style="color:var(--txt3);font-size:11px">None tracked</span>`;
  const _VIEWER="https://view.shelfwatch.io/?url=";
  const _vUrl=l=>l.startsWith(_VIEWER)?l:_VIEWER+encodeURIComponent(l);
  const _mkLinks=(links,vis)=>{
    if(!links||!links.length) return `<span style="color:var(--txt3);font-size:11px">No images</span>`;
    return links.map((l,i)=>{
      const seen=vis.includes(l);
      return `<a class="ilink${seen?" visited":""}" data-url="${esc(l)}" data-idx="${i+1}" href="${esc(_vUrl(l))}" target="_blank" rel="noopener">${seen?"":"🔗 "}Shelf image ${i+1}</a>`;
    }).join("");
  };
  let imgSection;
  if(HAS_AFTER && viewMode==="both"){
    const aftSt=afterStoreMap[s.name];
    const aftLinks=aftSt?aftSt.links||[]:[];
    imgSection=`<div style="font-size:10px;font-weight:700;letter-spacing:.5px;text-transform:uppercase;color:var(--teal);margin-bottom:4px">Before</div>${_mkLinks(s.links,visited)}<div style="font-size:10px;font-weight:700;letter-spacing:.5px;text-transform:uppercase;color:var(--amber);margin-top:10px;margin-bottom:4px">After</div>${_mkLinks(aftLinks,visited)}`;
  } else {
    imgSection=_mkLinks(s.links,visited);
  }
  document.getElementById("drill-content").innerHTML=`
    <div class="drill-name">${esc(s.name)}</div>
    <div class="drill-meta">ID: ${esc(String(s.shop_id||"—"))} · ${esc(s.date||"—")} · ${s.n_visits} total visit(s) · ${s.n_images} image(s)</div>
    ${stBar}<div class="action-row">${doneBtn}${flagBtn}</div>
    <div id="flag-area">${flagDisplay}</div>
    <div class="ds"><div class="ds-title">Own-Brand SKUs present (${s.n_self_skus})</div>${skuPills}</div>
    <div class="ds"><div class="ds-title">Competitor SKUs present (${s.n_comp_skus})</div>${compPills}</div>
    <div class="ds"><div class="ds-title">Images</div><div id="drill-links">${imgSection}</div></div>`;
  document.querySelectorAll("#drill-links .ilink").forEach(a=>{
    a.addEventListener("click",()=>{ qcVisitLink(visitKey(s),a.dataset.url); a.classList.add("visited"); a.textContent="Shelf image "+a.dataset.idx; updateProgress(); });
  });
}
function doDrillDone(){  if(!_drillStore)return; qcDone(visitKey(_drillStore));  renderDrill(_drillStore); renderStoreTable(); updateProgress(); }
function doDrillUndo(){  if(!_drillStore)return; qcUndo(visitKey(_drillStore));  renderDrill(_drillStore); renderStoreTable(); updateProgress(); }
function doDrillUnflag(){ if(!_drillStore)return; qcUnflag(visitKey(_drillStore)); renderDrill(_drillStore); renderStoreTable(); updateProgress(); }
function showFlagForm(){
  const s=_drillStore; if(!s)return;
  document.getElementById("flag-area").innerHTML=`<div id="flag-form">
    <div style="font-size:11px;color:var(--red);font-weight:700;margin-bottom:6px">🚩 Describe the issue</div>
    <textarea id="flag-note" placeholder="e.g. Images blurry, wrong store, data mismatch…"></textarea>
    <div class="flag-actions">
      <button class="flag-save" onclick="saveFlagForm()">Save Flag</button>
      <button class="flag-cancel" onclick="renderDrill(_drillStore)">Cancel</button>
    </div></div>`;
  document.getElementById("flag-note").focus();
}
function saveFlagForm(){
  if(!_drillStore)return;
  qcFlag(visitKey(_drillStore),(document.getElementById("flag-note")||{}).value||"");
  renderDrill(_drillStore); renderStoreTable(); updateProgress();
  showToast("Issue flagged");
}
function closeDrill(){ document.getElementById("drill").classList.remove("open"); _drillStore=null; }

// ── after section ─────────────────────────────────────────────────────────────
function renderAfter(){
  const el=document.getElementById("after-sec");
  if(!HAS_AFTER){
    el.innerHTML=`<div class="pending"><div class="pending-icon">⏳</div>
      <div class="pending-title">After-Data Pending</div>
      <div class="pending-desc">Upload an After file to unlock Before vs. After gap analysis, store-level deltas, and SKU improvement tracking.</div></div>`;
    return;
  }
  const sign=D.avail_delta>=0?"+":"", dCls=D.avail_delta>=0?"bg":"br";
  const gainers=[...D.sku_deltas].filter(x=>x.delta>0).sort((a,b)=>b.delta-a.delta).slice(0,10);
  const losers=[...D.sku_deltas].filter(x=>x.delta<0).sort((a,b)=>a.delta-b.delta).slice(0,10);
  const rows=(arr,pos)=>arr.map(s=>`<tr><td class="td-name">${esc(s.sku)}</td><td>${s.before}%</td><td>${s.after}%</td><td><span class="badge ${pos?"bg":"br"}">${pos?"+":""}${s.delta}pp</span></td></tr>`).join("");
  el.innerHTML=`<div class="card">
    <div class="sec-hdr"><div class="sec-title">Gap Analysis — Before vs. After</div></div>
    <div class="kpi-row" style="margin-bottom:16px">
      <div class="kpi-card accent"><div class="kpi-lbl">Before</div><div class="kpi-val" style="color:var(--amber)">${D.avail_before}%</div></div>
      <div class="kpi-card accent"><div class="kpi-lbl">After</div><div class="kpi-val" style="color:var(--amber)">${D.avail_after}%</div></div>
      <div class="kpi-card"><div class="kpi-lbl">Delta</div><div class="kpi-val"><span class="badge ${dCls} " style="font-size:22px;padding:4px 12px">${sign}${D.avail_delta}pp</span></div></div>
    </div>
    <div class="charts-row">
      <div><div class="card-title" style="margin-bottom:9px">Top Gainers</div>
        <div class="tbl-wrap"><table><thead><tr><th>SKU</th><th>Before</th><th>After</th><th>Delta</th></tr></thead><tbody>${rows(gainers,true)}</tbody></table></div></div>
      <div><div class="card-title" style="margin-bottom:9px">Top Losers</div>
        <div class="tbl-wrap"><table><thead><tr><th>SKU</th><th>Before</th><th>After</th><th>Delta</th></tr></thead><tbody>${rows(losers,false)}</tbody></table></div></div>
    </div></div>`;
}
...
function renderChips(){
  ...
  if(q)     h+=mkChip(`"${q}"`,()=>{q="";document.getElementById("s-search").value="";go();});
  if(skuF)  h+=mkChip(skuF.substring(0,18)+"…",()=>{skuF="";document.getElementById("s-sku").value="";go();});
  if(rangeF) h+=mkChip(rangeF+" SKUs",()=>{rangeF="";document.getElementById("s-range").value="";go();});
  if(qcF)   h+=mkChip("QC: "+qcF,()=>{qcF="";document.getElementById("s-qc").value="";go();syncKpiRings();});
  if(histBucket!==null){
    const bLabel=(histBucketView==="after"&&HAS_AFTER&&viewMode==="both"?"After ":"Before ")+"SKUs="+histBucket;
    h+=mkChip(bLabel,()=>{histBucket=null;histBucketView="before";refreshHist();go();});
  }
  el.innerHTML=h||`<span style="color:var(--txt3);font-size:11px">No filters active</span>`;
}
function mkChip(lbl,fn){
  const id="cf"+(_cf.push(fn)-1);
  setTimeout(()=>{ const el=document.getElementById(id); if(el) el.querySelector(".chip-x").addEventListener("click",()=>{fn();renderChips();}); },0);
  return `<span class="chip" id="${id}">${lbl} <span class="chip-x">✕</span></span>`;
}
function resetAll(){
  q="";skuF="";rangeF="";qcF="";histBucket=null;histBucketView="before";
  ["s-search","s-sku","s-range","s-qc"].forEach(id=>{ const el=document.getElementById(id); if(el) el.value=""; });
  refreshHist(); go(); syncKpiRings();
}

// ─────────────────────────────────────────────────────────────────────────────
// UPLOAD MODAL
// ─────────────────────────────────────────────────────────────────────────────
let _upFiles = {before: null, after: null, before_gal: null, after_gal: null};
let _upHeaders = [];

function openUpload(){ document.getElementById("upload-overlay").classList.remove("hidden"); resetUpProgress(); }
function closeUpload(){
  if(B) document.getElementById("upload-overlay").classList.add("hidden");
}
function resetUpProgress(){
  document.getElementById("up-prog-wrap").style.display="none";
  document.getElementById("up-prog-after").style.display="none";
  ["before","after"].forEach(w=>{
    const fill=document.getElementById("up-prog-fill-"+w);
    fill.style.width="0%"; fill.classList.remove("indeterminate");
    document.getElementById("up-prog-label-"+w).textContent=w==="before"?"Before":"After";
  });
  document.getElementById("up-note").textContent="";
}
...
function dzDrop(e,which){ e.preventDefault(); dzLeave("fz-"+which); const f=e.dataTransfer.files[0]; if(f) handleFile(f,which); }
function fileChosen(input,which){ if(input.files[0]) handleFile(input.files[0],which); }

function handleFile(file, which){
  _upFiles[which] = file;
  const fz = document.getElementById("fz-"+which);
  fz.classList.add("has-file");
  document.getElementById("fn-"+which).textContent = file.name;

  if(_upFiles.before){
    getHeadersViaWorker(_upFiles.before).then(headers=>{
      _upHeaders = headers;
      showColMapper(headers);
      document.getElementById("up-go-btn").disabled = false;
    }).catch(err=>{ showToast("Could not read file: "+err.message); });
  }
}

// ── background parsing worker (keeps UI thread free during large-file uploads) ─
function makeWorker(){
  return new Worker(URL.createObjectURL(new Blob([WORKER_SRC], {type:"text/javascript"})));
}

function getHeadersViaWorker(file){
  return new Promise((resolve,reject)=>{
    const w = makeWorker();
    w.onmessage = e=>{
      const m = e.data;
      if(m.type==="headers"){ resolve(m.headers); w.terminate(); }
    ...
    w.postMessage({cmd:"headers", file});
  });
}

function processFileViaWorker(file, colMap, galleryFile, onPhaseCb, onProgressCb){
  return new Promise((resolve,reject)=>{
    const w = makeWorker();
    w.onmessage = e=>{
      const m = e.data;
      if(m.type==="phase") onPhaseCb(m);
      else if(m.type==="progress") onProgressCb(m.done, m.total);
      else if(m.type==="done"){ resolve(m.kpis); w.terminate(); }
      else if(m.type==="error"){ reject(new Error(m.message)); w.terminate(); }
    };
    w.onerror = err=>{ reject(new Error(err.message||"Worker error")); w.terminate(); };
    w.postMessage({cmd:"process", file, colMap, galleryFile: galleryFile||null});
  });
}

const PHASE_LABEL = {reading:"Reading file…", "reading-gallery":"Reading gallery…", computing:"Computing…"};
function onPhase(which, m){
  document.getElementById("up-prog-wrap").style.display = "flex";
  const fill = document.getElementById("up-prog-fill-"+which);
  fill.classList.add("indeterminate");
  document.getElementById("up-prog-label-"+which).textContent = PHASE_LABEL[m.phase] || m.phase;
}
function onProgress(which, done, total){
  const fill = document.getElementById("up-prog-fill-"+which);
  fill.classList.remove("indeterminate");
  fill.style.width = (total ? Math.min(100, Math.round(100*done/total)) : 0)+"%";
  document.getElementById("up-prog-label-"+which).textContent =
    `Computing (${done.toLocaleString()} / ${total.toLocaleString()} rows)…`;
}
function progDone(which){
  const fill = document.getElementById("up-prog-fill-"+which);
  fill.classList.remove("indeterminate");
  fill.style.width = "100%";
  document.getElementById("up-prog-label-"+which).textContent = "Done";
}

function showColMapper(headers){
  document.getElementById("col-mapper").style.display="block";
  const none='<option value="">— none —</option>';
  const opts = none + headers.map(h=>`<option value="${esc(h)}">${esc(h)}</option>`).join("");

  ["cm-store","cm-sku","cm-brand","cm-date","cm-osa","cm-link","cm-id"].forEach(id=>{
    document.getElementById(id).innerHTML = opts;
  });

  const lower = headers.map(h=>h.toLowerCase());
  // Exact-match pass first (across all keywords), then substring fallback —
  // avoids e.g. "category_id" beating "category_name" just because it sorts earlier.
  const pick = (...keywords) => {
  ...
  document.getElementById("cm-store").value  = pick("shop_name","store_name","outlet_name","store","outlet");
  document.getElementById("cm-sku").value    = pick("class_name","gt_class_name","display_name","category_name","product_name","item_name","sku_name","sku","product","item");
  document.getElementById("cm-brand").value  = pick("competition","brand_type","brand","segment","type");
  document.getElementById("cm-date").value   = pick("audit_date","image_date","visit_date","date","period");
  document.getElementById("cm-osa").value    = pick("on_shelf_availability","on_shelf","osa","availability","present");
  document.getElementById("cm-link").value   = pick("image_link","photo_link","image_url","link","url","photo");
  document.getElementById("cm-id").value     = pick("client_shop_id","client_store_id","shop_id","store_id","id");
  document.getElementById("cm-self-val").value = "self";
  document.getElementById("cm-comp-val").value  = "competitor";
}

function getColMap(){
  return {
    store:      document.getElementById("cm-store").value,
    sku:        document.getElementById("cm-sku").value,
    brand:      document.getElementById("cm-brand").value,
    date:       document.getElementById("cm-date").value,
    osa:        document.getElementById("cm-osa").value,
    link:       document.getElementById("cm-link").value,
    storeId:    document.getElementById("cm-id").value,
    selfVal:    document.getElementById("cm-self-val").value.trim() || "self",
    compVal:    document.getElementById("cm-comp-val").value.trim() || "competitor",
  };
}
...
async function processUpload(){
  ...
  document.getElementById("up-prog-after").style.display = _upFiles.after ? "flex" : "none";
  try{
    const colMap = getColMap();
    if(!colMap.store||!colMap.sku||!colMap.brand||!colMap.date)
      throw new Error("Please map all required columns (*)");

    // Runs in a Worker so the UI thread stays responsive and the progress bar keeps
    // animating instead of the tab looking frozen. Before and After run one at a time,
    // not concurrently — two ~9M-cell parses at once thrash memory and end up *slower*
    // in total wall time than running them back to back (measured ~3x slower in testing).
    const beforeKpis = await processFileViaWorker(_upFiles.before, colMap, _upFiles.before_gal,
      m=>onPhase("before",m), (done,total)=>onProgress("before",done,total));
    progDone("before");

    let afterKpis = null;
    if(_upFiles.after){
      afterKpis = await processFileViaWorker(_upFiles.after, colMap, _upFiles.after_gal,
        m=>onPhase("after",m), (done,total)=>onProgress("after",done,total));
      progDone("after");
    }
    const hasAfter = !!afterKpis;
    const delta = hasAfter ? computeDeltaJS(beforeKpis, afterKpis) : null;

    closeUpload();
    loadDataset(beforeKpis, afterKpis||null, delta, hasAfter);
    showToast("Data loaded — "+beforeKpis.total_stores.toLocaleString()+" stores");
  } catch(err){
...
function computeDeltaJS(before, after){
  // Multiple rows per store — aggregate by max n_self_skus across visits
  const bm={},am={};
  before.stores.forEach(s=>{ bm[s.name]=Math.max(bm[s.name]||0,s.n_self_skus); });
  after.stores.forEach(s=>{  am[s.name]=Math.max(am[s.name]||0,s.n_self_skus);  });
  const allNames=new Set([...Object.keys(bm),...Object.keys(am)]);
  const store_deltas=Array.from(allNames).map(n=>({
    name:n, before:bm[n]||0, after:am[n]||0, delta:(am[n]||0)-(bm[n]||0),
  })).sort((a,b)=>a.delta-b.delta);
  const sb={},sa={};
  before.self_skus.forEach(s=>sb[s.sku]=s.rate);
  after.self_skus.forEach(s=>sa[s.sku]=s.rate);
  const allSkus=new Set([...Object.keys(sb),...Object.keys(sa)]);
  const sku_deltas=Array.from(allSkus).map(u=>({
    sku:u, before:sb[u]||0, after:sa[u]||0, delta:Math.round(10*((sa[u]||0)-(sb[u]||0)))/10,
  })).sort((a,b)=>a.delta-b.delta);
  return {
    avail_before:before.overall_avail_rate, avail_after:after.overall_avail_rate,
    avail_delta:Math.round(10*(after.overall_avail_rate-before.overall_avail_rate))/10,
    store_deltas, sku_deltas,
  };
}
...
<!-- SheetJS for xlsx browser parsing (loads from CDN when internet available) -->
...
# ── Web Worker source (parsing + KPI compute, off the UI thread) ──────────────
# SheetJS is prepended to this at generation time (extracted from the inline
# <script> block above) so the worker is fully self-contained in one Blob — no
# importScripts, no separate file to host.
WORKER_LOGIC_JS = r"""
self.onmessage = async function(e){
  const {cmd} = e.data;
  try{
    if(cmd==="headers"){
      const headers = await readHeaders(e.data.file);
      self.postMessage({type:"headers", headers});
    } else if(cmd==="process"){
      const {file, colMap, galleryFile} = e.data;
      self.postMessage({type:"phase", phase:"reading"});
      const records = await readFullFileW(file);
      let galleryRecords = [];
      if(galleryFile){
        self.postMessage({type:"phase", phase:"reading-gallery"});
        galleryRecords = await readFullFileW(galleryFile);
      }
      self.postMessage({type:"phase", phase:"computing", total:records.length});
      const kpis = await computeKpisJS(records, colMap, galleryRecords, (done,total)=>{
        self.postMessage({type:"progress", done, total});
      });
      self.postMessage({type:"done", kpis});
    }
  } catch(err){
...
// Mirror Python load_xlsx: pick the sheet with the most non-empty header columns
function _bestSheet(wb){
  let bestName=wb.SheetNames[0], bestCount=0;
  for(const sn of wb.SheetNames){
    const ws=wb.Sheets[sn];
    if(!ws||!ws['!ref']) continue;
    const rows=XLSX.utils.sheet_to_json(ws,{header:1,defval:""});
    const n=(rows[0]||[]).filter(v=>String(v).trim()!=="").length;
    if(n>bestCount){bestCount=n;bestName=sn;}
  }
...
async function readHeaders(file){
  ...
  const ws = _bestSheet(wb);
  const rows = XLSX.utils.sheet_to_json(ws, {header:1, defval:""});
  return (rows[0]||[]).map(String);
}

function parseCsvW(text){
  ...
  for(let i=1;i<lines.length;i++){
    ...
    const rec = {}; headers.forEach((h,j)=>rec[h]=vals[j]||"");
    records.push(rec);
  }
  return records;
}

async function readFullFileW(file){
  const isCsv = file.name.toLowerCase().endsWith(".csv");
  if(isCsv){ const text = await file.text(); return parseCsvW(text); }
  const buffer = await file.arrayBuffer();
  // dense:true stores rows as arrays instead of {A1:cell,B1:cell,...} objects — without it,
  // large sheets (millions of cells) hit V8's property-count limit and SheetJS silently
  // returns an empty sheet instead of throwing.
  const wb = XLSX.read(buffer, {type:"array", dense:true});
  const ws = _bestSheet(wb);
  return XLSX.utils.sheet_to_json(ws, {defval:""});
}

// ── JS KPI computation (mirrors Python compute_kpis, with OSA support) ────────
async function computeKpisJS(records, colMap, galleryRecords=[], onProgress=null){
  const _VIEWER="https://view.shelfwatch.io/?url=";
  const _vUrl=l=>l.startsWith(_VIEWER)?l:_VIEWER+encodeURIComponent(l);
  function _normDate(v){
    ...
    return s.slice(0,10)||"Unknown";
  }
  function _jsDate(s){
    if(!s||s==="Unknown") return new Date(0);
    if(s.includes("T")) return new Date(s);
    const m=s.match(/^(\d{1,2})\s([A-Za-z]+)\s(\d{2,4})$/);
    if(m){const mo={Jan:0,Feb:1,Mar:2,Apr:3,May:4,Jun:5,Jul:6,Aug:7,Sep:8,Oct:9,Nov:10,Dec:11};
      const y=+m[3]+(m[3].length===2?2000:0); return new Date(y,mo[m[2].slice(0,3)],+m[1]);}
    const d=new Date(s); return isNaN(d)?new Date(0):d;
  }
  function setAdd(obj,k,v){ if(!obj[k])obj[k]=new Set(); obj[k].add(v); }
  function setAddN(obj,k1,k2,v){ if(!obj[k1])obj[k1]={}; if(!obj[k1][k2])obj[k1][k2]=new Set(); obj[k1][k2].add(v); }
  function listAddN(obj,k1,k2,v,max){ if(!obj[k1])obj[k1]={}; if(!obj[k1][k2])obj[k1][k2]=[]; const l=obj[k1][k2]; if(l.length<max&&!l.includes(v))l.push(v); }

  const storeMeta={}, storeDates={};
  const sdSelf={}, sdComp={}, sdImgs={}, sdLinks={};
  const skuStores={}, compSkuStores={}, dateTotal={}, dateHasSelf={};
  const allSelf=new Set(), allComp=new Set();

  let _ri=0;
  for(const r of records){
    if(++_ri%10000===0){ if(onProgress) onProgress(_ri,records.length); await new Promise(res=>setTimeout(res,0)); }
    const store   = String(r[colMap.store]||"Unknown");
    const dateStr = _normDate(r[colMap.date]);
    const brandVal= String(r[colMap.brand]||"");
    const gt      = String(r[colMap.sku]||"");
    const rawLink = colMap.link?String(r[colMap.link]||""):"";
    const link    = rawLink?_vUrl(rawLink):"";
    const osaRaw  = colMap.osa?r[colMap.osa]:null;
    const isPresent= osaRaw===null?true:(String(osaRaw)==="1"||osaRaw===1);
    const imgId   = String(r[colMap.storeId]||"")||dateStr+store;

    if(!storeMeta[store]){
      // Try client_shop_id first (OSA schema), fall back to mapped storeId
      const cid=String(r['client_shop_id']||r[colMap.storeId]||"");
      storeMeta[store]={shop_id:r[colMap.storeId]||null, client_id:cid||null};
    }
    setAdd(storeDates,store,dateStr);
    setAdd(dateTotal,dateStr,store);
    setAddN(sdImgs,store,dateStr,imgId);
    if(link) listAddN(sdLinks,store,dateStr,link,Infinity);

    const isSelf=brandVal===colMap.selfVal, isComp=brandVal===colMap.compVal;
    if(isSelf&&gt&&gt!=="Others"){
      allSelf.add(gt);
      if(isPresent){ setAddN(sdSelf,store,dateStr,gt); setAdd(skuStores,gt,store); setAdd(dateHasSelf,dateStr,store); }
    } else if(isComp&&gt&&gt!=="Others"){
      allComp.add(gt);
      if(isPresent){ setAddN(sdComp,store,dateStr,gt); setAdd(compSkuStores,gt,store); }
    }
  }

  // Build gallery lookups from optional gallery dump file
  const galLinksByCid={}, galImagesByCid={}, galLinksByName={}, galImagesByName={};
  if(galleryRecords.length){
    const gKeys=Object.keys(galleryRecords[0]||{});
    const _gc=(...kws)=>{ for(const kw of kws){ const k=gKeys.find(k=>k.toLowerCase().includes(kw.toLowerCase())); if(k) return k; } return ""; };
    const gColCid  = _gc('client_store_id','client_id','store_id');
    const gColName = _gc('store_name','shop_name','store');
    const gColLink = _gc('image_link','link','url');
    const gColId   = _gc('image_id','id');
    const gColDate = _gc('audit_date','image_date','date');
    for(const g of galleryRecords){
      const gDate=_normDate(g[gColDate]), vurl=g[gColLink]?_vUrl(String(g[gColLink])):"";
      const cid=String(g[gColCid]||""), nm=String(g[gColName]||""), imgId=String(g[gColId]||"");
      if(!vurl) continue;
      if(cid){ listAddN(galLinksByCid,cid,gDate,vurl,Infinity); if(imgId)setAddN(galImagesByCid,cid,gDate,imgId); }
      if(nm){  listAddN(galLinksByName,nm,gDate,vurl,Infinity);  if(imgId)setAddN(galImagesByName,nm,gDate,imgId); }
    }
  }

  const total=Object.keys(storeMeta).length;
  const sn=Array.from(allSelf).sort(), cn=Array.from(allComp).sort();
  const si={}, ci={};
  sn.forEach((s,i)=>si[s]=i); cn.forEach((s,i)=>ci[s]=i);

  // One row per (store, date) visit — mirrors Python compute_kpis
  const stores=[];
  for(const [store,meta] of Object.entries(storeMeta)){
    const dates=Array.from(storeDates[store]||[]).sort((a,b)=>_jsDate(a)-_jsDate(b));
    const storeNVisits=dates.length;
    const cid=String(meta.client_id||"");
    for(const d of dates){
      const vSelf=(sdSelf[store]||{})[d]||new Set();
      const vComp=(sdComp[store]||{})[d]||new Set();

      // Gallery images: prefer client_id+date lookup, fall back to name+date, then OSA-embedded links
      let dateLnks=(sdLinks[store]||{})[d]||[];
      let dateImgSize=(sdImgs[store]||{})[d]?.size||0;
      if(galleryRecords.length){
        const glByCid=cid?(galLinksByCid[cid]||{})[d]||[]:[];
        const glByNm=(galLinksByName[store]||{})[d]||[];
        const giCid=cid?(galImagesByCid[cid]||{})[d]?.size||0:0;
        const giNm=(galImagesByName[store]||{})[d]?.size||0;
        if(glByCid.length){dateLnks=glByCid;dateImgSize=giCid;}
        else if(glByNm.length){dateLnks=glByNm;dateImgSize=giNm;}
      }

      stores.push({
        name:store, date:d, dates:[d], last_date:d,
        shop_id:meta.shop_id, client_id:meta.client_id,
        n_self_skus:vSelf.size, n_comp_skus:vComp.size,
        n_images:dateImgSize, n_visits:storeNVisits,
        si:Array.from(vSelf).sort().map(s=>si[s]).filter(x=>x!==undefined),
        ci:Array.from(vComp).sort().map(s=>ci[s]).filter(x=>x!==undefined).slice(0,10),
        links:dateLnks.slice(0,3),
        all_links:dateLnks,
        self_skus:Array.from(vSelf).sort(),
        comp_skus:Array.from(vComp).sort().slice(0,10),
      });
    }
  }
  stores.sort((a,b)=>b.n_self_skus-a.n_self_skus||(a.name<b.name?-1:1));

  const selfSkus=Object.entries(skuStores).sort((a,b)=>b[1].size-a[1].size)
    .map(([sku,set])=>({sku,stores:set.size,rate:Math.round(1000*set.size/total)/10}));
  const compSkus=Object.entries(compSkuStores).sort((a,b)=>b[1].size-a[1].size)
    .map(([sku,set])=>({sku,stores:set.size,rate:Math.round(1000*set.size/total)/10}));

  const dist={};
  stores.forEach(s=>{ dist[s.n_self_skus]=(dist[s.n_self_skus]||0)+1; });
  const histogram=Object.entries(dist).sort((a,b)=>+a[0]-+b[0]).map(([bucket,count])=>({bucket:+bucket,count}));

  const allDates=Object.keys(dateTotal).sort((a,b)=>_jsDate(a)-_jsDate(b));
  const date_trend=allDates.map(d=>({
    date:d,
    stores_visited:(dateTotal[d]||new Set()).size,
    stores_with_brunch:(dateHasSelf[d]||new Set()).size,
    rate:Math.round(1000*(dateHasSelf[d]||new Set()).size/Math.max(1,(dateTotal[d]||new Set()).size))/10,
  }));

  // Unique stores with ≥1 self SKU on any visit
  const sw=Object.keys(storeMeta).filter(store=>
    Array.from(storeDates[store]||[]).some(d=>(sdSelf[store]||{})[d]?.size>0)
  ).length;

  return {
    project_name:"Availability Report",
    date_range:allDates.length?allDates[0]+" – "+allDates[allDates.length-1]:"",
    total_stores:total, total_visits:stores.length,
    stores_with_brunch:sw, stores_no_brunch:total-sw,
    overall_avail_rate:Math.round(1000*sw/Math.max(1,total))/10,
    total_self_skus:allSelf.size, total_comp_skus:allComp.size,
    sku_names:sn, comp_names:cn,
    stores, self_skus:selfSkus, comp_skus:compSkus, histogram, date_trend,
  };
}
"""
...
def main():
    ...
    # Reuse the SheetJS source already inline in HTML (single source of truth) to build
    # a self-contained Worker blob: SheetJS + WORKER_LOGIC_JS, no importScripts needed.
    sheetjs_start = HTML.index("<script>/*! xlsx.js") + len("<script>")
    sheetjs_end = HTML.index("</script>", sheetjs_start)
    sheetjs_src = HTML[sheetjs_start:sheetjs_end]
    worker_src = json.dumps(sheetjs_src + "\n" + WORKER_LOGIC_JS, ensure_ascii=False)

    html = HTML.replace("__DATA_JSON__", json.dumps(shell_payload, ensure_ascii=False))
    html = html.replace("__WORKER_SRC_JSON__", worker_src)
    ...
    tool_html = tool_html.replace("__WORKER_SRC_JSON__", worker_src)
